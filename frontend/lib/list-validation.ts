...
export function validMatchList(value: unknown, results = false): boolean {
  ...
  return value.every((row) => {
    ...
    if (!results) {
      if (!["scheduled", "pending", "live"].includes(String(row.status))) return false;
      if (!["live_home", "live_away"].every((key) => score(row[key]))) return false;
      if (row.live_minute !== null && (typeof row.live_minute !== "string"
        || !/^(?:\d{1,3}(?:\+\d{1,2})?|HT)$/.test(row.live_minute))) return false;
      if (row.score_checked_at !== null && (typeof row.score_checked_at !== "string"
        || !Number.isFinite(Date.parse(row.score_checked_at)))) return false;
      return row.status !== "live" || row.live_home !== null && row.live_away !== null;
    }
